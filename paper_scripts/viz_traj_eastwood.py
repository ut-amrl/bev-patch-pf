"""Visualize the eastwood dataset train/val/test split on a map."""
...
LINE_PROPS = {
    "train": {"color": "#0072B0", "alpha": 0.8, "linewidth": 4, "offset": (0, 0)},
    "val": {"color": "#009E70", "alpha": 0.8, "linewidth": 4, "offset": (0, 0)},
    "test1": {"color": "#FFAC1C", "alpha": 0.8, "linewidth": 4, "offset": (0, 0)},
    "test2": {"color": "#FD3DB5", "alpha": 0.8, "linewidth": 4, "offset": (0, 0)},
}


def viz_traj_eastwood(args):
    # Load dataset config
    with open(args.config) as f:
        dataset_cfg = yaml.safe_load(f)

    tiff_path = dataset_cfg["geo_tiff_path"].replace("${.root}", args.root)
    geo_handler = GeoTiffHandler(tiff_path)

    fig, ax = plt.subplots(figsize=(20, 20))
    ax.imshow(geo_handler.image[:600, :1200], alpha=0.5)
    ...
    for split_option, label in ORDERED_LABELS:
        ...
        for scene_name in scene_names:
            ...
            # odometry data
            odom_file = scene_dir / "utm_pose.csv"
    ...
    draw_scale_bar(ax, resolution, length_m=100, pad=3.0, font_size=30)
...
def draw_scale_bar(ax, m_per_pixel, length_m, pad, font_size: int = 20, color="black"):
    ...
    bar_box.add_artist(Line2D([0, 0], [0, -30], color=color, linewidth=5))
    bar_box.add_artist(Line2D([bar_px, bar_px], [0, -30], color=color, linewidth=5))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--root", type=str, default="data/ARL_Jackal")
    parser.add_argument("--config", type=str, default="config/dataset/arl_jackal_eastwood.yaml")
    parser.add_argument("--out_path", type=str, default="paper_scripts/figure/eastwood_split.png")
    args = parser.parse_args()

    viz_traj_eastwood(args)
