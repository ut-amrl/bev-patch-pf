...
def get_legend_opts(config: dict) -> tuple[bool, int, str]:
    viz_cfg = config.get("viz", {})
    if viz_cfg is None:
        viz_cfg = {}
    if not isinstance(viz_cfg, dict):
        raise ValueError("Config key 'viz' must be a mapping when provided.")

    legend_cfg = viz_cfg.get("legend", {})
    if legend_cfg is None:
        legend_cfg = {}
    if not isinstance(legend_cfg, dict):
        raise ValueError("Config key 'viz.legend' must be a mapping when provided.")

    show_legend = bool(legend_cfg.get("show", True))
    legend_fontsize = int(legend_cfg.get("fontsize", 25))
    if legend_fontsize <= 0:
        raise ValueError("viz.legend.fontsize must be > 0.")
    legend_loc = str(legend_cfg.get("loc", "upper left"))
    if not legend_loc.strip():
        raise ValueError("viz.legend.loc must be a non-empty string.")
    return show_legend, legend_fontsize, legend_loc
...
def clip_trajectory(t: np.ndarray, traj: list[SE2], t_min: float, t_max: float):
    """Clip (t, traj) to [t_min, t_max] inclusive."""
    mask = (t >= t_min) & (t <= t_max)
    idx = np.nonzero(mask)[0]
    return t[mask], [traj[i] for i in idx]
...
def evaluate_scene(scene: str, methods: list[dict], geo_handler: GeoTiffHandler):
    ...
    if ref_method_cfg:
        try:
            ...
            logger.info(f"  Reference points: {ref_num_points}")
    ...
    for method_cfg in methods:
        ...
        try:
            ...
            if ref_t is not None and ref_traj is not None:
                n_before = len(est_t)
                est_t, est_traj = clip_trajectory(est_t, est_traj, ref_t[0], ref_t[-1])
                logger.info(f"  Clipped {method_name}: {n_before} -> {len(est_t)} poses")
                if len(est_t) == 0:
                    logger.warning(f"  Skipping {method_name}: no overlapping timestamps with reference")
                    continue
            ...
            if ref_t is not None and ref_traj is not None:
                ...
                if len(pos_errors) == 0:
                    logger.warning(f"  Skipping {method_name}: no interpolatable timestamps with reference")
                    continue
                ate_num_points = len(pos_errors)

                # Store errors for CDF plotting
                errors[method_name] = {
                    "pos_errors": pos_errors,
                    "ori_errors": ori_errors,
                    "ate_num_points": ate_num_points,
                }
                ...
                logger.info(f"  {method_name}: points={ate_num_points}, ATE={ate_rmse:.3f}m, AOE={aoe_rmse:.2f}°")
            else:
...
def plot_trajectories(trajectories: dict, geo_handler: GeoTiffHandler, config: dict, save_path: Path):
    ...
    base_linewidth = float(viz_cfg.get("linewidth", 5.0))
    reference_linewidth = float(viz_cfg.get("reference_linewidth", max(8.0, base_linewidth)))
    if base_linewidth <= 0 or reference_linewidth <= 0:
        raise ValueError("viz.linewidth and viz.reference_linewidth must be > 0.")
    ...
    for name, (_, traj) in trajectories.items():
        ...
        linewidth = reference_linewidth if name == reference_name else base_linewidth
        ax.plot(
            pixels[:, 0] - x_min,
            pixels[:, 1] - y_min,
            label=name,
    ...
    # Legend
    show_legend, legend_fontsize, legend_loc = get_legend_opts(config)
    if show_legend:
...
def plot_error_cdf(
    ...
    _, _, legend_loc = get_legend_opts(config)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", default="paper_scripts/config/experiment/tartandrive.yaml")
