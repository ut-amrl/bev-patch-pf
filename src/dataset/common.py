...
class GeoLocDataset(Dataset, ABC):
    ...
    CAM2ENU: torch.Tensor = None  # camera pose in robot's baselink frame (ENU)
    ...
    def get_aerial_image(self, gt_xyr: np.ndarray, geo_handler: GeoTiffHandler | None = None):
        geo_handler = self.geo_handler if geo_handler is None else geo_handler
        if geo_handler is None:
            raise ValueError("GeoTIFF handler is required to sample aerial imagery.")
        ...
        reframer = AerialReframer.from_query_coords(
            geo_handler, query_coords, out_size=self.aerial_image_resize, scale=scale, rotation=rotation
        )
    ...
    def __getitem__(self, idx):
        ...
        data.update({"aerial_image": aerial_image, "reframer": reframer, "gt_uvr": gt_uvr})
        data["info"].update({"resolution": resolution, "rotation": rotation})

        if self.sample_poses:
            pose_xyr = reframer.sample_poses(
                gt_xyr, self.n_poses, self.dist_bounds, self.r_bounds, radial_power=self.radial_power
            )
            pose_uvr = torch.tensor(reframer.to_uvr(pose_xyr), dtype=torch.float32)
            data.update({"pose_xyr": torch.tensor(pose_xyr, dtype=torch.float32), "pose_uvr": pose_uvr})
