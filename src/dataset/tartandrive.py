...
from dataset.utils import compute_se3_actions, se3_poses_from_rows
...
class TartanDriveSequence(TartanDriveDataset):
    def __init__(self, root: str, scene: str, **kwargs):
        ...
        # Motion updates from camera-frame visual odometry.
        odom_path = Path(root) / scene / "pycuvslam.csv"
        odom_data = np.genfromtxt(odom_path, delimiter=",", skip_header=1)
        odom_poses = se3_poses_from_rows(odom_data[:, 1:8], camera_frame=True, cam2enu=self.CAM2ENU)
