...
class GeoTiffHandler:
    ...
    def pixel_to_coords(self, uv: np.ndarray) -> np.ndarray:
        x, y = self.geotransform * (uv[..., 0], uv[..., 1])
...
class AerialReframer:
    ...
    def to_coords(self, uv: np.ndarray) -> np.ndarray:
        """Convert output-image UV coordinates to projected world/map coordinates."""
        uv_xy = uv[..., :2].astype(np.float32, copy=False)
        u_rot, v_rot = uv_xy[..., 0], uv_xy[..., 1]
        H_out, W_out = self.out_size

        if self.rotation == 90:
            uv_unrot = np.stack([(W_out - 1) - v_rot, u_rot], axis=-1)
        elif self.rotation == 180:
            uv_unrot = np.stack([(W_out - 1) - u_rot, (H_out - 1) - v_rot], axis=-1)
        elif self.rotation == 270:
            uv_unrot = np.stack([v_rot, (H_out - 1) - u_rot], axis=-1)
        else:
            uv_unrot = uv_xy

        uv_global_px = uv_unrot * self.scale + np.array([self.u1, self.v1], dtype=np.float32)

        return self.geo_handler.pixel_to_coords(uv_global_px)
