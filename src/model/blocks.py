...
def conv3x3(in_planes: int, out_planes: int, stride: int = 1, bias=False) -> nn.Conv2d:
...
class ResidualBlock(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, stride: int = 1) -> None:
        ...
        if stride != 1 or in_channels != out_channels:
            self.skip = conv1x1(in_channels, out_channels, stride=stride)
        else:
            self.skip = nn.Identity()

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        out = self.conv1(self.act1(self.gn1(x)))
        out = self.conv2(self.act2(self.gn2(out)))
        out = out + self.skip(x)
        return out
