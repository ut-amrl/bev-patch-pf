...
from utils.visualization import PCA, depth_to_rgb, features_to_rgb
...
class RerunLogger:
    ...
    def __init__(
        ...
        **_: object,
        ...
        if rr is None:
            raise RuntimeError("`rerun-sdk` is required for visualization. Install project dependencies and try again.")
        ...
        self.pca_states: dict[str, PCA] = {}
        self.trajectory_log_interval = 10
        self.show_features = show_features

        # ReRun logger
        blueprint_path = Path(blueprint_path).expanduser() if blueprint_path is not None else None
        if blueprint_path is not None and not spawn:
            raise ValueError("blueprint_path requires spawn=True")

        if blueprint_path is not None:
            if not blueprint_path.exists():
                raise FileNotFoundError(f"Rerun blueprint file not found: {blueprint_path}")

            rerun_exe = shutil.which("rerun")
            if rerun_exe is None:
                raise RuntimeError("`rerun` executable not found. Install rerun-sdk so the viewer binary is available.")

            try:
                subprocess.Popen(
                    [rerun_exe, str(blueprint_path)],
                ...
                raise RuntimeError(f"Failed to launch rerun with blueprint `{blueprint_path}`.") from exc
        else:
            try:
                rr.init(app_id, spawn=spawn)
            except Exception as exc:
                raise RuntimeError(f"Failed to initialize rerun live viewer (spawn={spawn}).") from exc

        if not spawn and output_dir is None:
            raise ValueError("output_dir is required when spawn=False")
    ...
    def log(self, frame: dict, results: dict) -> None:
        ...
        self._log_trajectory("map/gt", gt_uvr, COLORS["gt"], length=20.0, radii=3.0, draw_order=1)
        est_uvr = self.geo_handler.coords_to_uvr(est_xyr)
        self._log_trajectory("map/est", est_uvr, COLORS["est"], length=20.0, radii=3.0, draw_order=3)
        odom_uvr = self.geo_handler.coords_to_uvr(odom_xyr)
        self._log_trajectory("map/odom", odom_uvr, COLORS["odom"], length=20.0, radii=3.0, draw_order=2)
        ...
        self._log_arrows("map/particles", particles_uvr, color=COLORS["particles"], length=5.0, radii=1.0)
        ...
        self._log_arrows("local/gt", local_gt_uvr, color=COLORS["gt"], length=40.0, radii=10.0, draw_order=1)
        self._log_arrows("local/est", local_est_uvr, color=COLORS["est"], length=40.0, radii=10.0, draw_order=3)
        self._log_arrows("local/odom", local_odom_uvr, color=COLORS["odom"], length=40.0, radii=10.0, draw_order=2)
        ...
        if self.show_features and ("aer_raw_feat" in results) and ("bev_feat" in results):
            self._log_feature(
                results["bev_feat"].detach().squeeze(0),
                results["aer_raw_feat"].detach().squeeze(0),
                tags=["bev/feature", "local/aerial_feature"],
                upsample_scales=[1, 4],
                latent_space="bev",
            )
    ...
    def _log_scalar(self, tag: str, value: float, color: tuple[int, int, int], width: float = 1.0):
        rr.log(tag, rr.Scalars(value), rr.SeriesLines(colors=color, widths=width))
    ...
    def _log_feature(
        ...
        *features: torch.Tensor,
        ...
        if latent_space not in self.pca_states:
            self.pca_states[latent_space] = PCA(n_components=3)

        feature_rgbs = features_to_rgb(*features, pca=self.pca_states[latent_space])
