...
from utils.misc import as_scalar, load_checkpoint, load_model, save_checkpoint
...
def train_one_epoch(model, loader, criterion, optimizer, scheduler, accelerator, cfg, epoch):
    ...
    for idx, batch in enumerate(
        tqdm(loader, desc=f"Train [{epoch}/{cfg.epochs}]", total=epoch_steps, disable=not is_main)
    ):
        if idx >= epoch_steps:
            break
...
def evaluate(model, loader, accelerator, cfg, metrics=None, prefix: str = "eval") -> dict:
    model.eval()
...
def build_loader(cfg):
    ...
    val_loader = DataLoader(
        val_dataset, batch_size=cfg.batch_size, shuffle=False, collate_fn=safe_collate, num_workers=0, pin_memory=True
    )
...
def main(cfg: DictConfig):
    ...
    model = load_model(cfg.model, cfg.ckpt_path, device=accelerator.device)
    ...
    accelerator.init_trackers(
        ...
        init_kwargs={"wandb": {"name": f"{cfg.dataset_name}@{cfg.model.name}"}},
    )
